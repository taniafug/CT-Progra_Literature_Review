...
from stage_1_import_filtering.review_pipeline_logic_updated_before import (
    DEFAULT_DATABASES,
    run_stage1_pipeline,
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(f"Default temp folder: `{TEMP_DIR_1}`")
    ...
    if run_button:
        ...
        metric_4.metric(
            ...
            len(results["abstract_screening_input"]),
        )
...
"""
        ...
        with st.expander("Abstract screening input", expanded=True):
            st.dataframe(
                results["abstract_screening_input"].astype(str),
                use_container_width=True,
            )
