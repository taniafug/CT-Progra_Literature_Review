...
def clean_text(value: Any) -> str:
    """General text normalization for non-title fields."""
    if pd.isna(value):
        return ""
...
def clean_title(title: Any) -> str:
    ...
    The important difference from clean_text() is that apostrophes are removed,
    not converted to spaces. This makes variants such as children's, children’s,
    and childrens match consistently. Hyphens and other punctuation still become
    spaces so problem-solving and problem solving also match.
    """
    if pd.isna(title):
        return ""
...
def clean_doi(doi: Any) -> str:
    """Normalize DOI strings from database exports."""
    if pd.isna(doi):
        return ""
...
def _build_exact_duplicate_components(df: pd.DataFrame) -> list[dict]:
    """Build connected duplicate groups across DOI, title, and abstract keys.

    This avoids the previous sequential problem where a record already marked as
    duplicate was removed from later title/abstract checks, which could leave a
    transitive duplicate in the clean data.
    """
    if df.empty or "Record_ID" not in df.columns:
        return []

    parent = {rid: rid for rid in df["Record_ID"].astype(str)}
    ...
    match_sources: dict[tuple[str, str], list[str]] = {}

    for key_col, reason in [
        ("DOI_Clean", "DOI exact match"),
        ("Title_Clean", "Title normalized match"),
        ...
        for key_value, group in valid.groupby(key_col, dropna=False):
            ...
            match_sources[(key_col, str(key_value))] = record_ids
    ...
    for record_ids in components.values():
        ...
        component_set = set(record_ids)
        reasons = []
        keys = []

        for (key_col, key_value), key_record_ids in match_sources.items():
            overlap = component_set.intersection(key_record_ids)
            if len(overlap) > 1:
                label = {
                    "DOI_Clean": "DOI exact match",
                    "Title_Clean": "Title normalized match",
                ...
                keys.append(f"{key_col}: {key_value}")

        group_df = df[df["Record_ID"].astype(str).isin(record_ids)].copy()
        if "Completeness_Score" not in group_df.columns:
            group_df["Completeness_Score"] = group_df.apply(calculate_completeness_score, axis=1)
        ...
        duplicate_components.append(
            {
                ...
                "keep_id": keep_id,
                "remove_ids": sorted([rid for rid in record_ids if rid != keep_id]),
...
def build_exact_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    working = df.copy()
    if "Completeness_Score" not in working.columns:
        working["Completeness_Score"] = working.apply(calculate_completeness_score, axis=1)

    for group_number, component in enumerate(_build_exact_duplicate_components(working), start=1):
        rows.append(
            {
                "Duplicate_Group_ID": f"D{group_number:04d}",
                "Duplicate_Type": "; ".join(component["reasons"]),
                "Match_Value": " | ".join(component["keys"]),
                "Record_IDs": "; ".join(component["record_ids"]),
                "Retained_Record_ID": component["keep_id"],
                "Removed_Record_IDs": "; ".join(component["remove_ids"]),
                "Count": len(component["record_ids"]),
                "Removed_Count": len(component["remove_ids"]),
            }
        )
...
def mark_exact_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    components = _build_exact_duplicate_components(out)

    for group_number, component in enumerate(components, start=1):
        group_id = f"D{group_number:04d}"
        record_ids = component["record_ids"]
        keep_id = component["keep_id"]
        remove_ids = component["remove_ids"]
        reason = "; ".join(component["reasons"])

        out.loc[out["Record_ID"].astype(str).isin(record_ids), "Duplicate_Group_ID"] = group_id
        out.loc[out["Record_ID"].astype(str).isin(record_ids), "Duplicate_Reason"] = reason
        out.loc[out["Record_ID"].astype(str) == keep_id, "Duplicate_Status"] = "Retained"
        out.loc[out["Record_ID"].astype(str).isin(remove_ids), "Duplicate_Status"] = "Duplicate"
...
def build_prisma_counts(
    ...
    rows = [
        ...
        {"Stage": "Records sent to abstract screening", "Count": len(stage_2_input)},
...
def run_stage1_pipeline(uploaded_files_by_database: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    ...
    stage_2_input = build_abstract_screening_input(title_screening_results)
...
def write_stage1_output_workbooks(results: Dict[str, pd.DataFrame], output_dir: Path) -> Dict[str, Path]:
    ...
    processing_path = output_dir / "stage_1_full_analysis.xlsx"
    title_path = output_dir / "stage_1_results.xlsx"
    abstract_input_path = output_dir / "stage_2_input.xlsx"
    ...
    write_workbook_to_disk(
        abstract_input_path,
        {
            "Abstract_Screening_Input": results["stage_2_input"],
        },
        styled=True,
    )
