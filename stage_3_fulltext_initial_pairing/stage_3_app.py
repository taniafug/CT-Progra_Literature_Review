...
STAGE_4_DIR = PROJECT_ROOT / "stage_4_full_pairing"
STAGE_4_INPUT_DIR = STAGE_4_DIR / "input"
STAGE_4_INPUT_PATH = STAGE_4_INPUT_DIR / "stage_4_input.xlsx"
STAGE_3_TO_STAGE_4_OUTPUT_PATH = OUTPUT_DIR_3 / "stage_3_to_stage_4_input.xlsx"
...
NO_FULL_TEXT_PATH = (OUTPUT_DIR_3 / "stage_3_no_full_text_available.xlsx")

from stage_3_logic import (
...
DEFAULT_RESULTS_OUTPUT_NAME = "stage_3_results.xlsx"
DEFAULT_VALIDATION_OUTPUT_NAME = "stage_3_author_validation.xlsx"
DEFAULT_COSTS_OUTPUT_NAME = "stage_3_costs.xlsx"
...
# ============================================================
def main(): 
    st.set_page_config(page_title="Stage 3 – Full-Text Screening with LLM", layout="wide")
    st.title("Stage 3 – Full-Text Screening with LLM")
    ...
    default_input_path = (INPUT_DIR_3 / "stage_3_input.xlsx")
    ...
    run_button = st.button("Run Stage 3 full-text screening", type="primary")
    ...
    if run_button:
        ...
        for i, (_, row_data) in enumerate(filtered_df.iterrows(), start=1):
            ...
            if skip_reason:
                ...
                outputs = write_checkpoint_excel(
        ...
        # ============================================================
        # Prepare Stage 4 input as a Stage 3 output and copy it to Stage 4
        # ============================================================
        try:
            next_stage_df = outputs.get("next_stage", pd.DataFrame())

            with pd.ExcelWriter(STAGE_3_TO_STAGE_4_OUTPUT_PATH, engine="openpyxl") as writer:
                next_stage_df.to_excel(
                    writer,
                    sheet_name="Next_stage_included",
                    index=False,
                )

            STAGE_4_INPUT_DIR.mkdir(parents=True, exist_ok=True)
            shutil.copy2(
                STAGE_3_TO_STAGE_4_OUTPUT_PATH,
                STAGE_4_INPUT_PATH,
            )

            st.success("Stage 4 input prepared successfully.")
            st.caption(f"Stage 3 handoff output: {STAGE_3_TO_STAGE_4_OUTPUT_PATH}")
            st.caption(f"Copied to Stage 4 input: {STAGE_4_INPUT_PATH}")

        except Exception as e:
            st.error(f"Could not prepare Stage 4 input from Stage 3 output: {e}")
        ...
        • {STAGE_3_TO_STAGE_4_OUTPUT_PATH.name}
