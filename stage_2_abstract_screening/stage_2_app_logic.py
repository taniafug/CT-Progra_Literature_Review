...
def build_decision_summary_df(
    ...
    return pd.DataFrame(
        [
            {"Category": "Safety duplicates removed before abstract screening", "Count": removed_duplicates_count,},
...
def resolve_pdf_repository_dir(pdf_repository_dir: str | Path | None = None) -> Path:
    """Return the Stage 3 PDF repository folder used by the handoff file."""
    if pdf_repository_dir is not None and str(pdf_repository_dir).strip():
        return Path(pdf_repository_dir).expanduser()

    return (
        get_script_folder().parent
...
def ensure_pdf_paths_for_next_stage(
    next_stage_df: pd.DataFrame,
    pdf_repository_dir: str | Path | None = None,
) -> pd.DataFrame:
    """Create PDF_Filename, PDF_Path, and File_Exists for every Stage 3 record.

    Stage 3 needs a concrete expected PDF path for each paper even when the
    file has not been downloaded yet. Existing non-empty PDF_Path values are
    preserved; missing ones are rebuilt from the Stage 3 PDF repository and
    PDF_Filename.
    """
    if next_stage_df is None or next_stage_df.empty:
        return pd.DataFrame() if next_stage_df is None else next_stage_df.copy()

    out = next_stage_df.copy()
    repository_dir = resolve_pdf_repository_dir(pdf_repository_dir)
    repository_dir.mkdir(parents=True, exist_ok=True)

    if "Full_Text_ID" not in out.columns:
        if "Record_ID" in out.columns:
            id_series = out["Record_ID"].fillna("").astype(str).str.strip()
        elif "Paper_ID" in out.columns:
            id_series = out["Paper_ID"].fillna("").astype(str).str.strip()
        else:
            id_series = pd.Series(
                [f"P{i:05d}" for i in range(1, len(out) + 1)],
                index=out.index,
            )
            out.insert(0, "Paper_ID", id_series)
        ...
        out["Full_Text_ID"] = id_series.apply(safe_filename)

    if "PDF_Filename" not in out.columns:
        out["PDF_Filename"] = out["Full_Text_ID"].astype(str).apply(safe_filename) + ".pdf"
    else:
        filename_series = out["PDF_Filename"].fillna("").astype(str).str.strip()
        missing_filename = filename_series.isin(["", "nan", "none", "null"])
        out.loc[missing_filename, "PDF_Filename"] = (
            out.loc[missing_filename, "Full_Text_ID"]
            .astype(str)
            .apply(safe_filename)
            + ".pdf"
        )

    if "PDF_Path" not in out.columns:
        out["PDF_Path"] = ""

    path_series = out["PDF_Path"].fillna("").astype(str).str.strip()
    missing_path = path_series.isin(["", "nan", "none", "null"])

    out.loc[missing_path, "PDF_Path"] = out.loc[missing_path, "PDF_Filename"].apply(
        lambda filename: str(repository_dir / str(filename))
    )

    out["File_Exists"] = out["PDF_Path"].apply(
        lambda path: Path(str(path)).expanduser().exists()
        if str(path).strip().lower() not in ["", "nan", "none", "null"]
        else False
    )

    if "PDF_Retrieval_Status" not in out.columns:
        out["PDF_Retrieval_Status"] = "Pending"
    else:
        status_series = out["PDF_Retrieval_Status"].fillna("").astype(str).str.strip()
        missing_status = status_series.isin(["", "nan", "none", "null"])
        out.loc[missing_status, "PDF_Retrieval_Status"] = "Pending"

    exists_mask = out["File_Exists"].astype(bool)
    pending_mask = out["PDF_Retrieval_Status"].fillna("").astype(str).str.strip().isin(
        ["", "Pending", "nan", "none", "null"]
    )
    out.loc[exists_mask & pending_mask, "PDF_Retrieval_Status"] = "Already exists"

    if "PDF_Retrieval_Source" not in out.columns:
        out["PDF_Retrieval_Source"] = ""
    if "PDF_Retrieval_Message" not in out.columns:
        out["PDF_Retrieval_Message"] = ""
...
def build_all_output_dataframes(
    ...
    # - Other_Count is hidden from the main decision tabs requested by the user.
    # - Abstract_Quality_Note is hidden only from No_Manual_Review.
    included_df = included_df.drop(columns=["Other_Count"], errors="ignore")
    excluded_df = excluded_df.drop(columns=["Other_Count"], errors="ignore")
    undetermined_df = undetermined_df.drop(columns=["Other_Count"], errors="ignore")
    no_manual_review_df = no_manual_review_df.drop(
        columns=["Other_Count", "Abstract_Quality_Note"],
        errors="ignore",
    )
...
def write_reorganized_output_workbooks(
    ...
    results_output_path = output_dir / "stage_2_results.xlsx"
    validation_output_path = output_dir / "stage_2_author_validation.xlsx"
    costs_output_path = output_dir / "stage_2_costs.xlsx"
    next_stage_output_path = output_dir / "stage_2_stage_3_input.xlsx"
    ...
    pdf_repository_path = resolve_pdf_repository_dir(pdf_repository_dir)

    dfs["Next_Stage_Fulltext"] = ensure_pdf_paths_for_next_stage(
        next_stage_df=dfs["Next_Stage_Fulltext"],
        pdf_repository_dir=pdf_repository_path,
    )
...
def run_abstract_screening(
    ...
    reset_excel_pointer(abstract_file)
    df, original_count, deduplicated_count, removed_duplicates_count = load_and_prepare_abstracts(
    ...
    base_output_path = output_dir / "stage_2.xlsx"

    checkpoint_path = temp_dir / "stage_2_checkpoint.xlsx"
